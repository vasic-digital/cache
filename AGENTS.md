...
Inherited verbatim from the parent project's `/CLAUDE.md` §6.X (added 2026-05-13 in response to the operator's twenty-first §6.L invocation: "when we rely / depend on emulator(s) needed for the testing of the System, make sure we boot up Container running Android emulator in it using ours Containers Submodule. It is supported and it works, it just need proper connecting into the flows."). Every Android emulator instance MUST execute INSIDE a podman/docker container managed by `Submodules/Containers/`. Host-direct emulator launches are permitted for workstation iteration only; the constitutional gate run (release tagging, real-device verification) MUST go through the container-bound path. `pkg/runtime/` brings the container up; `pkg/emulator/` orchestrates the AVD lifecycle inside it. §6.X-debt tracks the wiring implementation owed to the Containers submodule. This submodule MAY add stricter rules but MUST NOT relax.
...
**(B) Decoupling / reusability.** Owned submodules MUST remain fully decoupled from the consuming project. No project-specific context, hardcoded paths, hostnames, asset names, or runtime assumptions may be introduced into an owned submodule's source tree. When a submodule needs information from the consuming project, the honest path is configuration injection (env var, config file, constructor parameter) — never a hardcoded reach into the parent's tree. Every owned submodule MUST be project-not-aware, fully reusable, modular, and completely testable as a standalone repository.
...
- `<repo_root>/<submodule_name>/` (flat layout — the current project layout for Challenges, HelixQA, Containers, Security, etc.)
...
## Inherited covenant restatements — Helix §11.4.98 + §11.4.99 (inherited per §11.4.35)

These two mandates are inherited from HelixConstitution via parent-discovery (§11.4.35). This file **restates + cites** them — does NOT redefine or weaken. The literal anchors `11.4.98` and `11.4.99` are required by the §11.4.87 `CM-COVENANT-114-87-PROPAGATION` pre-build gate which asserts they appear in every consuming repo's CLAUDE.md / AGENTS.md / QWEN.md.

### §11.4.98 — Full-Automation Anti-Bluff Mandate (Helix, 2026-05-28)

Every test this Constitution governs — unit/integration/e2e/Challenge/stress/chaos/live — MUST be fully self-driving end-to-end with NO human action during execution. A test requiring manual action during execution is **by definition a §11.4 PASS-bluff at the automation layer**. Single permissible exception: one-time credential bootstrap OUTSIDE test execution (`.env`/`~/.bashrc`/OAuth/session activation — configuration, not test driving). Concrete: no "operator MUST type a message" prompts; no dev-session-UUID collisions; no 60s human-response windows (§11.4.50 violation); PASS at `-count=3` consecutive automated runs with self-cleaning state; every existing test classified COMPLIANT vs NON-COMPLIANT; no false-positive PASS — silent-skip-as-PASS forbidden, §11.4.3 SKIP-with-reason is correct. Composes with §11.4.85 + §11.4.89 + §11.4.87 + §11.4.94. Canonical authority: HelixConstitution Constitution.md §11.4.98 (inherited per §11.4.35). Non-compliance is a release blocker.

### §11.4.99 — Latest-Source Documentation Cross-Reference Mandate (Helix, 2026-05-28)

Every operator-facing instruction document MUST be cross-referenced against the LATEST official online documentation of every external service/library it touches BEFORE the commit lands. Misguidance-by-stale-docs is a §11.4 PASS-bluff at the documentation layer. Anchoring case study (2026-05-28): a Telegram MTProto setup guide recommended VoIP + omitted `recover@telegram.org` pre-login email + used underscores in Telegram's strict-alphanumeric short_name field; all three contradicted official Telegram + gotd/td docs and could have caused permanent account bans. (A) Pre-commit workflow: fetch LATEST official docs via WebFetch/MCP/direct browsing (NEVER training data); cite source URL+date in `## Sources verified` doc footer AND commit-message footer; seek secondary authoritative sources for sparse official. (B) Negative findings documented explicitly. (C) Re-verification: 6-month default staleness, 90-day for risk-classified services (messengers/cloud/payment/AI-LLM/code-hosting/package-managers). (D) Risk-classified services require explicit safety warnings vs latest policies. (E) Composes with §11.4.92 Pass 4 INDEPENDENT. (F) Inheritance per §11.4.35, literal anchor `11.4.99`, pre-build gate `CM-COVENANT-114-99-PROPAGATION`, paired §1.1 mutations strip → gates FAIL. (G) Enforcement: missing footer → release-gate BLOCK; stale-beyond-grace → §11.4.90 Obsolete `Reason=stale-documentation`. Canonical authority: HelixConstitution Constitution.md §11.4.99 (inherited per §11.4.35). Non-compliance is a release blocker.
